...
**Severity: High. FIXED in `f2de15b`.** `ChallengeManager::challenge` now panics
`settlement_deadline_passed` for **every** proof type once
`Registry::get_cert_settlement_deadline(cert_id)` has been reached — which is
the rail this section suggests, applied at filing, and widened past
`InsufficientReserve` because the deadline's meaning is not proof-type-specific
and the freeze is the harm. A window opened before the deadline is unaffected:
the freeze it writes becomes the live deadline, so joining claims still pass and
it settles normally. The cost — a genuine breach discovered after the deadline
is unchallengeable — is stated in DESIGN-V2 §9 and §10. The PoC below is kept
under its own name, converted to assert the attack now fails; the companion
`a_window_opened_before_the_deadline_still_admits_claims_and_settles` covers the
half the fix must not break.
...
**Severity: High. FIXED in `f2de15b`** — but not by any of the three directions
suggested at the foot of this section. All three (a non-refundable filing fee,
following distinct victim addresses, capping claims per window) price the attack
or invite a new one; none removes it, because `n` is the attacker's free
parameter and addresses are free.

The fix is to stop asking the predicate a question it cannot answer. A
predicate-computed proof establishes that **the covenant was broken**, not that
any named person was harmed — which is exactly why `BoundExceeded` and
`ExpiredCertificate` already settled with no victim payment. So
`InsufficientReserve` now pays **no victim compensation**: the operator's
reserve draw goes to the treasury, the challenger still earns their fee, and
compensation flows only from arbiter-assessed harm, the one mechanism that can
name and size a victim. The victim pot is divided by **assessed** harm, so a
predicate claim cannot shave an assessed victim's share either.

**The cost, stated plainly:** the protocol's only trustless proof no longer
compensates anyone directly, so proving harm now always requires the arbiter.
That narrows the trustless surface and widens R4, and it is recorded in
DESIGN-V2 §1, §9 and §10.

**Not closed, deliberately:** sybils still split the challenger **fee**. That
pot is a bounty for surfacing a fact rather than compensation for a loss, its
total is fixed at 10% of proven harm however many file, and the same trade is
already made on purpose by the flat hygiene bounty. See DESIGN-V2 §10.

Demonstrated:
`sybil_claims_dilute_an_honest_victim_and_every_sybil_bond_comes_back`,
converted to assert the swarm now takes nothing from an assessed victim, plus
...
| #   | Finding                                                         | Severity | Demonstrated      | Status              |
| --- | --------------------------------------------------------------- | -------- | ----------------- | ------------------- |
...
R1 and D1 compounded: D1 left the auditor exposed at attestation, and R1 left
them exposed at the end of the certificate's life. Between them, an auditor's
allocation was destroyable by a third party at both ends of a certificate that
never did anything wrong. Both ends are now closed — D1 by the `attest` reserve
check, R1 by the settlement-deadline rail — and, taken together, they mean an
attested certificate's reserve is **monotone until its deadline**, so there is
no window in which a third party can manufacture a shortfall proof at all.

T1 does not reproduce: every contract bumps its instance entry on every write
path and extends the persistent entries it creates (defect L2, DESIGN-V2
"Storage lifetime"). The review flagged it explicitly as a suspicion rather than
a demonstration, and it is left in the table so the check is on record.
