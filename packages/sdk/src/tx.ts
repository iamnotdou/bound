// Transaction builders for the connected-wallet path.
//
...
//
// SERVER ONLY — this is the server half of the split, like config.ts: it
// imports ./config and reaches the chain (RPC + Horizon). Browser code must not
// import it; client code takes `@bound/sdk/deployments` for addresses instead.
//
// No secrets live here — the wallet signs. The permissionless `resolve` step
// (challenger flow) is signed server-side via BoundClient.resolveChallenge.
import {
  rpc,
  Horizon,
  TransactionBuilder,
